...
public class RobotContainer {
    ...
    private final SwerveRequest.FieldCentric drive = new SwerveRequest.FieldCentric()
            .withDeadband(MaxSpeed * percentDeadband)
            .withRotationalDeadband(MaxAngularRate * percentDeadband)
    ...
    public RobotContainer() {
        ...
        NamedCommands.registerCommand(
                "runIntakeForever",
                intake.runIntake().alongWith(shooter.runReverse()).asProxy());
        NamedCommands.registerCommand(
                "intakeFinish",
                intake.runInverseALittle().deadlineFor(shooter.runReverse()).asProxy());
        NamedCommands.registerCommand("runShooterForever", shooter.runForward().asProxy());
        NamedCommands.registerCommand(
                "shoot",
                intake.runIntake()
                        .alongWith(shooter.runForward())
                        .withTimeout(0.5)
                        .asProxy());
        NamedCommands.registerCommand(
                "shootSequence",
    ...
    private void configureBindings() {
        ...
        joystick.rightBumper().whileTrue(shooter.runForward());

        joystick.rightTrigger(0.5).and(joystick.rightBumper()).whileTrue(intake.runIntake());
